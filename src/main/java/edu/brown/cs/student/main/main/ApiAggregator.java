...
public class ApiAggregator {
    ...
    public List<Object> getData(String dataType) throws Exception {
        ...
        String response1 = client.makeRequest(ClientRequestGenerator.secureRequest(filename + "one?auth="));
        String response2 = client.makeRequest(ClientRequestGenerator.secureRequest(filename + "three?auth="));
        ...
        try{
            for(int i = 0; i < 5; i++){
                if(response.equals(error_message)){
                    response = client.makeRequest(ClientRequestGenerator.secureRequest(filename + server + "?auth="));
                }else break;
            }
